...
## 🎮 CV Quest & Progression Dashboard

### 🎯 앱 목표 (App Goals)
- **LearnOpenCV** 저장소 내 380개 이상의 컴퓨터 비전 및 딥러닝 실습 프로젝트를 체계적으로 학습할 수 있도록 돕는 **게이밍 퀘스트 대시보드 시스템**입니다.
- 스테이지 해금(Unlock), 경험치(XP) 및 레벨 업 시스템을 도입하여 각 프로젝트의 코드와 Jupyter 노트북을 순차적으로 탐험하고 로컬 환경에서 직접 실행하며 CV/AI 역량을 키울 수 있도록 지원합니다.

---

### 🏗️ 프로그램 구조 (Program Structure)
```
learnopencv/
├── learnopencv_progression.py       # 과제 스캔, 디렉토리 분류, 상태(State) 관리 CLI 엔진
├── generate_cvquest_notebooks.py   # 프로젝트별 실습용 Jupyter 래퍼 노트북(.ipynb) 자동 생성기
├── learnopencv_progression_web.py   # 로컬 HTTP 대시보드 서버 & API 백엔드 (캐싱 및 Jupyter 제어)
├── learnopencv_game.html            # 게이밍 인터랙티브 대시보드 UI (HTML/CSS/JS)
├── .cvquest/                        # CVQuest 자동 생성 데이터 및 Jupyter 로그 저장소
│   ├── notebooks/                   # 380+ 퀘스트별 실행 가능 래퍼 노트북 (.ipynb)
│   └── quest_notebooks.json         # 프로젝트 - 노트북 연동 매핑 파일
├── progress_state.json              # 사용자 과제 진행 상황 (locked / unlocked / completed)
└── progress_manifest.json           # 전체 과제 메타데이터 내보내기 파일
```
...
#### 1. 퀘스트 노트북 매핑 생성
저장소 내 모든 프로젝트의 실행 환경과 원본 코드를 연결하는 Jupyter 노트북을 생성합니다.
```bash
python generate_cvquest_notebooks.py
```

#### 2. CLI 대시보드 사용
터미널에서 직접 과제 상태를 확인하고 관리할 수 있습니다.
```bash
# 전체 과제 목록 출력
python learnopencv_progression.py list

# 특정 과제 상세 정보 보기 (예: ID 1)
python learnopencv_progression.py info 1

# 과제 완료 처리 (다음 과제 자동 해금)
python learnopencv_progression.py complete 1
...
#### 3. 웹 인터랙티브 대시보드 실행
웹 브라우저에서 게이밍 대시보드를 실행합니다.
```bash
python learnopencv_progression_web.py
```
서버 실행 후 브라우저에서 **`http://127.0.0.1:8000`** 접속.

---

### ⚙️ 배포 및 환경 구축 (Deployment & Setup)

#### 1. 기본 환경 요구사항
- Python 3.9 이상
- OpenCV (`opencv-python`), `numpy`, `matplotlib`, `nbformat`
...
# 필수 패키지 설치
pip install opencv-python numpy matplotlib nbformat
```

#### 3. JupyterLab 대시보드 연동 배포 (선택)
웹 대시보드에서 `Jupyter Start` 및 노트북 원클릭 열기 기능을 사용하려면 `.venv-jupyter` 환경에 `jupyterlab`을 설치합니다.
```bash
